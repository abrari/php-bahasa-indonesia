...
%token <ast> T_LNUMBER   "bilangan bulat (T_LNUMBER)"
%token <ast> T_DNUMBER   "bilangan float (T_DNUMBER)"
%token <ast> T_STRING    "identifier (T_STRING)"
%token <ast> T_VARIABLE  "variabel (T_VARIABLE)"
%token <ast> T_INLINE_HTML
%token <ast> T_ENCAPSED_AND_WHITESPACE  "string dikutip dan whitespace (T_ENCAPSED_AND_WHITESPACE)"
%token <ast> T_CONSTANT_ENCAPSED_STRING "string dikutip (T_CONSTANT_ENCAPSED_STRING)"
%token <ast> T_STRING_VARNAME "nama variabel (T_STRING_VARNAME)"
%token <ast> T_NUM_STRING "angka (T_NUM_STRING)"

%token END 0 "akhir dari file"
%token T_INCLUDE      "sertakan (T_INCLUDE)"
%token T_INCLUDE_ONCE "sertakan_satu_kali (T_INCLUDE_ONCE)"
%token T_EVAL         "jalankan (T_EVAL)"
%token T_REQUIRE      "butuh (T_REQUIRE)"
%token T_REQUIRE_ONCE "butuh_satu_kali (T_REQUIRE_ONCE)"
%token T_LOGICAL_OR   "atau (T_LOGICAL_OR)"
%token T_LOGICAL_XOR  "xor (T_LOGICAL_XOR)"
%token T_LOGICAL_AND  "dan (T_LOGICAL_AND)"
%token T_PRINT        "cetak (T_PRINT)"
%token T_YIELD        "hasilkan (T_YIELD)"
%token T_YIELD_FROM   "hasilkan dari (T_YIELD_FROM)"
...
%token T_SL "<< (T_SL)"
%token T_SR ">> (T_SR)"
%token T_INSTANCEOF  "merupakansuatu (T_INSTANCEOF)"
%token T_INC "++ (T_INC)"
%token T_DEC "-- (T_DEC)"
...
%token T_OBJECT_CAST "(objek) (T_OBJECT_CAST)"
%token T_BOOL_CAST   "(bool) (T_BOOL_CAST)"
%token T_UNSET_CAST  "(hapus) (T_UNSET_CAST)"
%token T_NEW       "buat (T_NEW)"
%token T_BARU      "baru (T_BARU)"
%token T_CLONE     "salin (T_CLONE)"
%token T_EXIT      "keluar (T_EXIT)"
%token T_IF        "jika (T_IF)"
%token T_ELSE      "selainnya (T_ELSE)"
%token T_ECHO       "tampil (T_ECHO)"
%token T_DO         "kerjakan (T_DO)"
%token T_WHILE      "selama (T_WHILE)"
%token T_FOR        "untuk (T_FOR)"
%token T_FOREACH    "untuksetiap (T_FOREACH)"
%token T_DECLARE    "deklarasi (T_DECLARE)"
%token T_AS         "sebagai (T_AS)"
%token T_DALAM      "dalam (T_DALAM)"
%token T_SWITCH     "pilihan (T_SWITCH)"
%token T_CASE       "kalau (T_CASE)"
%token T_DEFAULT    "default (T_DEFAULT)"
%token T_BREAK      "berhenti (T_BREAK)"
%token T_CONTINUE   "lanjut (T_CONTINUE)"
%token T_GOTO       "lompatke (T_GOTO)"
%token T_FUNCTION   "fungsi (T_FUNCTION)"
%token T_CONST      "konstanta (T_CONST)"
%token T_RETURN     "kembalikan (T_RETURN)"
%token T_TRY        "coba (T_TRY)"
%token T_CATCH      "tangkap (T_CATCH)"
%token T_FINALLY    "akhirnya (T_FINALLY)"
%token T_THROW      "lempar (T_THROW)"
%token T_USE        "gunakan (T_USE)"
%token T_INSTEADOF  "alihalih (T_INSTEADOF)"
%token T_GLOBAL     "global (T_GLOBAL)"
%token T_STATIC     "statis (T_STATIC)"
%token T_ABSTRACT   "abstrak (T_ABSTRACT)"
%token T_FINAL      "final (T_FINAL)"
%token T_PRIVATE    "privat (T_PRIVATE)"
%token T_PROTECTED  "terproteksi (T_PROTECTED)"
%token T_PUBLIC     "publik (T_PUBLIC)"
%token T_VAR        "variabel (T_VAR)"
%token T_UNSET      "hapus (T_UNSET)"
%token T_ISSET      "diset (T_ISSET)"
%token T_EMPTY      "kosong (T_EMPTY)"
%token T_HALT_COMPILER "__halt_compiler (T_HALT_COMPILER)"
%token T_CLASS      "kelas (T_CLASS)"
%token T_TRAIT      "sifat (T_TRAIT)"
%token T_INTERFACE  "interface (T_INTERFACE)"
%token T_EXTENDS    "turunan (T_EXTENDS)"
%token T_IMPLEMENTS "mengimplementasi (T_IMPLEMENTS)"
%token T_OBJECT_OPERATOR "-> (T_OBJECT_OPERATOR)"
%token T_DOUBLE_ARROW    "=> (T_DOUBLE_ARROW)"
%token T_LIST            "daftar (T_LIST)"
%token T_ARRAY           "larik (T_ARRAY)"
...
%token T_COMMENT         "komentar (T_COMMENT)"
%token T_DOC_COMMENT     "komentar doc (T_DOC_COMMENT)"
%token T_OPEN_TAG        "tag buka (T_OPEN_TAG)"
%token T_OPEN_TAG_WITH_ECHO "tag buka dengan echo (T_OPEN_TAG_WITH_ECHO)"
%token T_CLOSE_TAG       "tag tutup (T_CLOSE_TAG)"
%token T_WHITESPACE      "whitespace (T_WHITESPACE)"
%token T_START_HEREDOC   "awal heredoc (T_START_HEREDOC)"
%token T_END_HEREDOC     "akhir heredoc (T_END_HEREDOC)"
%token T_DOLLAR_OPEN_CURLY_BRACES "${ (T_DOLLAR_OPEN_CURLY_BRACES)"
%token T_CURLY_OPEN      "{$ (T_CURLY_OPEN)"
%token T_PAAMAYIM_NEKUDOTAYIM ":: (T_PAAMAYIM_NEKUDOTAYIM)"
%token T_NAMESPACE       "ruangnama (T_NAMESPACE)"
...
%type <num> non_empty_member_modifiers member_modifier
